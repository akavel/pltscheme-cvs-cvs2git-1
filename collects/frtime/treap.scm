...
; My prelude, myenv.scm, is assumed
;(include "myenv.scm")
...
(module treap (lib "plt-pretty-big.ss" "lang")
  ...
  (define (make-treap key-compare)
    ...
    (let ((root #f) (size 0))
      ...
      (define-macro (locate-extremum-node2 branch-selector)
      ...
      (define (debugprint)
        (cerr nl "The treap contains " size " nodes" nl)
        (let loop ((node root) (level 0))
          (when node
      ...
      (lambda (selector)
        (case selector
          ((enqueue) ; Greg Cooper's extensions
           (lambda (key val)
             (let ([node (locate-node key)])
               (if node
                   (node:value-set! node (cons val (node:value node)))
                   (insert! key (cons val empty))))))
          
          ((dequeue)
           (let* ([node (locate-extremum-node2 node:left-kid)]
                  [val (node:value node)])
             (if (empty? (rest val))
                 (delete-extremum-node! node:left-kid node:left-kid-set!
                                        node:right-kid)
                 (node:value-set! node (rest val)))
             (first val)))
