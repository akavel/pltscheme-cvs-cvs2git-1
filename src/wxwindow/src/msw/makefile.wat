...
EXTRAMODULES = $(FAFALIB) $(GAUGELIB) $(ITSYLIB) $(PROIOLIB) $(DIBLIB) $(RCPARSERLIB) $(WXSTRINGLIB) $(WXXPMLIB) # $(ODBCLIB) # $(CTL3DOBJ)
...
OBJECTS = wx_win.obj wx_frame.obj wx_panel.obj wx_utils.obj &
  ...
  wx_slidr.obj wx_menu.obj wx_db.obj wx_cmdlg.obj wx_main.obj
...
base:   .SYMBOLIC
    cd ..\base
    wmakel -f makefile.wat OPTIONS="$(OPTIONS)" DEBUG="$(DEBUG)" LEVEL=$(LEVEL)
    cd ..\msw

$(LIBTARGET) : $(OBJECTS) $(BASEOBJECTS) $(EXTRATARGETS) $(EXTRAMODULES) makefile.wat
    %create tmp.lbc
...
wxstring:   .SYMBOLIC
    ...
    wmake -f makefile.wat OPTIONS="$(OPTIONS)" DEBUG="$(DEBUG)"
    cd $(WXDIR)\src\msw
